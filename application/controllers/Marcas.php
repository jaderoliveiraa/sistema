...
class Marcas extends CI_Controller {
    ...
    public function index() {
        ...
        $data = array(
            ...
            'marcas' => $this->core_model->get_all('marcas'), // Pegar todas as SMarcas        
        );
    ...
    public function add() {
        $this->form_validation->set_rules('marca_nome', '', 'trim|required|required|min_length[2]|max_length[45]');
    ...
    public function edit($marca_id = NULL) {
        ...
        if (!$marca_id || !$this->core_model->get_by_id('marcas', array('marca_id' => $marca_id))) {
            ...
            $this->form_validation->set_rules('marca_nome', '', 'trim|required|required|min_length[4]|max_length[45]');
            ...
            if ($this->form_validation->run()) {
                ...
                if($this->db->table_exists('produtos')){
                    ...
                    if($marca_ativa == 0 && $this->core_model->get_by_id('produtos', array('produto_marca_id' =>$marca_id, 'produto_ativo' => 1))){
                        $this->session->set_flashdata('error', 'Esta Marca está em uso em PRODUTOS e não pode ser desativada!');
                        redirect('marcas');
