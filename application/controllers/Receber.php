...
class Receber extends CI_Controller {
    ...
    public function index() {
        ...
        $data = array(
            ...
            'contas_receber' => $this->financeiro_model->get_all_receber(''), // Pegar todas as contas a receber        
        );
    ...
    public function add() {
        ...
        if ($this->form_validation->run()) {
                ...
                $data = array(
                    ...
                    'clientes' => $this->core_model->get_all('clientes', array('cliente_ativo' => 1)), // Pegar todos os clientes ativos
                );
        ...
        if (!$conta_receber_id || !$this->core_model->get_by_Id('contas_receber', array('conta_receber_id' => $conta_receber_id))) {
            ...
            if ($this->form_validation->run()) {
                ...
                $data = array(
                    ...
                    'conta_receber' => $this->core_model->get_by_id('contas_receber', array('conta_receber_id' => $conta_receber_id)), // Pegar todas as contas a receber        
                    'clientes' => $this->core_model->get_all('clientes', array('cliente_ativo' => 1)), // Pegar todos os clientes ativos
                );
    ...
    public function del($conta_receber_id = NULL) {
        ...
        if (!$conta_receber_id || !$this->core_model->get_by_id('contas_receber', array('conta_receber_id' => $conta_receber_id))) {
            ...
            $this->session->set_flashdata('error', 'Esta conta não pode ser deletada pois ainda não foi paga!');
            redirect('receber');
        }
         else {
